...
impl Compiler {
    ...
    fn pre_process_functions(&mut self) {
        ...
        let mut non_imported_functions = vec![];
        for i in 0..self.function_defs.len() {
            if let TopLevelOperation::DefineFunction(function_def) = &self.function_defs[i] {
                ...
                non_imported_functions.push(function_def.name.clone());
        ...
        for i in 0..self.function_defs.len() {
            if let TopLevelOperation::DefineFunction(function_def) = &self.function_defs[i] {
                ...
                if let Some(x) = &function_def.external_name {
    ...
    fn create_data(&mut self, bytes: Vec<u8>) -> i32 {
        ...
        let size = bytes.len();
        self.wasm.add_data(Data::new(pos as u32, bytes));
            ...
            Expression::FunctionCall(x) => {
                if &x.function_name == "do" {
                    ...
                    if x.params.len() > 0 {
                        ...
                        for k in 0..x.params.len() {
                            if let Expression::FunctionCall(c) = &x.params[k] {
                                if c.function_name == "continue" {
                                    break;
                                }
                            }
                            self.process_expression(i, &x.params[k]);
