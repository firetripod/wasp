...
named!(expression<CompleteStr, Expression>,
    alt!(expression_let|empty_list|expression_number|boolean_true|boolean_false|expression_comment|expression_literal_string|expression_identifier|expression_function_call)
);
...
named!(define_wasm_function<CompleteStr, TopLevelOperation>,
  do_parse!(
    ...
    function_name: ws!(token_identifier) >>
    many0!(ws!(token_comment)) >>
    external_name:opt!( ws!(token_text)) >>
    many0!(ws!(token_comment)) >>
    ws!(tag!("["))   >>
    many0!(ws!(token_comment)) >>
...
named!(app<CompleteStr, App>,
  do_parse!(
    op: many0!(ws!(alt!(comment|external_function|define_wasm_function|define_function|define_global))) >>
    eof!() >>
    (App{children:op})
  )
);
